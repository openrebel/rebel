/* window.js is a vanilla javascript library, designed for Pro-test 4.0 and Revolt 1.0
 * Developed by Andreas Venizelou, 2021.
 * Released into the public domain.
 */

const w_ANIM_DURATION = 200;
const w_TOOLBAR_GAP   = 48;
...
iconsbar.onmousedown = event => {
    if (event.button == 1) event.preventDefault(); //prevent mid-mouse scroll

    if (event.buttons == 1 && event.layerX > iconsbar.clientWidth - 2)
...
class Window {
    constructor(themeColor = [56,56,56]) {
        ...
        iconsbar.appendChild(this.task);
    ...
    Close() {
        ...
        this.win.style.transition = w_ANIM_DURATION/1333 + "s";
        this.win.style.opacity    = "0";
        this.win.style.transform  = "scale(.85)";

        this.task.style.transition = w_ANIM_DURATION/2000 + "s";
        ...
        setTimeout(()=> {
            if (this.popoutWindow)
                ...
                iconsbar.removeChild(this.task);
    ...
    Toogle() {
        ...
        this.win.style.transition = w_ANIM_DURATION/1000 + "s";
        ...
        if (this.isMaximized) {
            ...
            this.toolbox.style.left      = w_TOOLBAR_GAP + "px";
            this.isMaximized = false;
    ...
    Minimize(force) {
        ...
        if (this.isMinimized && !force) { //restore
            ...
            this.isMinimized = false;
            setTimeout(()=> { this.BringToFront(); }, w_ANIM_DURATION/2);
        ...
        setTimeout(()=> { this.win.style.transition = "0s"; }, w_ANIM_DURATION);
    }
    ...
    Popout() {
        ...
        newWin.document.write("<link rel='icon' href='res/favicon.ico'>");
        newWin.document.write("<link rel='stylesheet' href='root.css'>");

        for (let i = 0; i < w_STYLES.length; i++)
            newWin.document.write(`<link rel='stylesheet' href='${w_STYLES[i]}'>`);
        ...
        btnUnpop.style.backgroundImage = "url(res/w_popout.svg)";
...
function Win_mousemove(event) {
    ...
    if ($w.isMoving) {
        ...
        x = Math.max(0, x);
        x = Math.min(iconsbar.clientWidth - $w.active.task.clientWidth, x);
...
function Win_mouseup(event) {
    ...
    if ($w.active != null) {
        $w.active.task.style.transition = w_ANIM_DURATION/1000 + "s";
...
function Win_AlignIcon(ignoreActive) {
    ...
    iconsbar.style.height = $w.iconSize + "px";
    container.style.bottom = $w.iconSize + "px";
    ...
    if (ignoreActive) {
        for (let i=0; i<temp.length; i++)
            if (temp[i] != $w.active.task) {
                temp[i].style.transition = w_ANIM_DURATION/1000 + "s";
